// Run Solution - ESP32 robot executor
// Solution string is updated by the cube solver (main.py). Flash this sketch,
// open Serial Monitor (115200), then press SPACE and ENTER to run the stored algorithm.
...
void setup() {
  ...
  Serial.println("--- Run Solution Ready ---");
  Serial.println("Press SPACE then ENTER to execute the stored algorithm.");
}
...
void moveFace(int motorIdx, int quarters, bool clockwise) {
  ...
  digitalWrite(motors[motorIdx][2], LOW);
  delay(10);
  ...
  delay(BRAKE_TIME);
  digitalWrite(motors[motorIdx][2], HIGH);
}
...
void loop() {
  if (Serial.available() > 0) {
    ...
    // Trigger: SPACE then ENTER (serial sends " \n")
    if (incoming.equals(" ")) {
      Serial.println("Running stored solution...");
      processCommand(String(SOLUTION));
    }
  }
}
